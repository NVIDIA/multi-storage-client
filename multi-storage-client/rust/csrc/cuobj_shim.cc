...
// cuObject client API from the installed NVIDIA cuObject SDK (CUDA Toolkit
// >= 13.1). It transitively pulls in cufile.h + cuobjtelem.h from the same
// include dir, so only the SDK include path is needed to build the shim.
