...
from multistorageclient.types import MAX_SYMLINK_DEPTH, ObjectMetadata, Range
...
class _OrdinaryObjectFileClient(_RangeClient):
    ...
    def download_file(self, _path: str, destination: io.BytesIO) -> None:
...
def test_get_file_forces_windowed_streaming_for_an_ordinary_object_file_provider() -> None:
    """fsspec downloads bypass ObjectFile's ordinary-provider eager-prefetch default."""
    content = b"0123456789"
    ...
    filesystem.get_file("objects/data.bin", destination, prefetch_file=True)
...
def test_get_file_path_does_not_truncate_an_existing_target_when_source_open_fails(tmp_path: Path) -> None:
    target = tmp_path / "target.bin"
    target.write_bytes(b"old-target")
    filesystem = _filesystem_for(_FailingOpenClient())
...
def test_get_file_path_keeps_an_existing_target_and_cleans_its_temp_file_when_streaming_fails(tmp_path: Path) -> None:
    target = tmp_path / "target.bin"
    target.write_bytes(b"old-target")
