...
def test_manager_mode_disabled_metrics_do_not_crash_record():
    """A disabled meter returned through a real manager proxy yields instrument proxies that still
    expose ``.set``/``.add`` (the proxy ``exposed`` method set is declared statically, independent of
    the wrapped ``None`` referent). Recording through them must not crash the storage operation: the
    record-time latch disables metrics after the first failure."""
