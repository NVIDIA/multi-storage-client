...
This file describes the project for any coding agent. Sub-projects (`multi-storage-client` and `multi-storage-explorer`) have their own `AGENTS.md` with language-specific details.
...
Multi-language project: **Python** (core) + **Rust** (performance).
...
| Python storage providers |  |
| Config schema | All layers (Python config parsing, Rust config) |
...
See `multi-storage-client/AGENTS.md` and `multi-storage-explorer/AGENTS.md` for language-specific commands.
