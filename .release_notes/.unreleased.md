...
- The MSAL OTLP span exporter now retries failed `POST` exports (with the same status forcelist as the metric exporter); urllib3's default retry policy excluded POST, so only connection errors were retried.
